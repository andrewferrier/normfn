...
class TestSubprocessArguments(NormalizeFilenameTestCase):
    ...
    def test_no_basicdateprefix(self):
        (rc, output, error) = self.invokeAsSubprocess(
            [], extraParams=["--help"], expectOutput=True
        )
    ...
    def test_recursive_current_directory(self):
        ...
        self.assertPathExists(
            os.path.join(newsubWorkingDir, self.getDatePrefix() + "foobar.txt")
        )
    ...
    def test_recursive_current_directory_interactive(self):
        ...
        self.assertPathExists(
            os.path.join(subWorkingDir, self.getDatePrefix() + "foobar.txt")
        )
    ...
    def test_recursive_current_directory_interactive_with_dotfile(self):
        ...
        self.assertPathExists(
            os.path.join(subWorkingDir, self.getDatePrefix() + "foo.txt")
        )
    ...
    def test_loads_of_files(self):
        ...
        for filename in filenames:
            ...
            self.assertPathExists(
                os.path.join(self.workingDir, self.getDatePrefix() + filename)
            )
