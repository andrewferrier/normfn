...
class TestSubprocessBasic(NormalizeFilenameTestCase):
    ...
    def test_basicdatemove_partial(self):
        ...
        self.assertFalse(os.path.exists(filename))
        self.assertTrue(
            os.path.exists(os.path.join(self.workingDir, "2015-01-blah.txt"))
        )
        self.assertEqual(1, self.directoryFileCount(self.workingDir))
        self.assertEqual("", output)
        self.assertEqual("", error)
    ...
    def test_datemove_partial(self):
        ...
        self.assertFalse(os.path.exists(filename))
        self.assertTrue(
            os.path.exists(os.path.join(self.workingDir, "2015-01-blah-bling.txt"))
        )
        self.assertEqual(1, self.directoryFileCount(self.workingDir))
        self.assertEqual("", output)
        self.assertEqual("", error)
    ...
    def test_basicdatemove2(self):
        ...
        self.assertFalse(os.path.exists(filename))
        self.assertTrue(
            os.path.exists(os.path.join(self.workingDir, "2015-01-01-blah-bling.txt"))
        )
        self.assertEqual(1, self.directoryFileCount(self.workingDir))
        self.assertEqual("", output)
        self.assertEqual("", error)
    ...
    def test_basicdatemove2_underscore(self):
        ...
        self.assertFalse(os.path.exists(filename))
        self.assertTrue(
            os.path.exists(os.path.join(self.workingDir, "2015-01-01-blah_bling.txt"))
        )
        self.assertEqual(1, self.directoryFileCount(self.workingDir))
        self.assertEqual("", output)
        self.assertEqual("", error)
    ...
    def test_basicdateprefix_interactive_yes(self):
        ...
        (rc, output, error) = self.invokeAsSubprocess(
            [filename], feedInput=b"y", extraParams=["--interactive"], expectOutput=True
        )
        self.assertEqual(0, rc)
        self.assertFalse(os.path.exists(filename))
        self.assertTrue(
    ...
    def test_basicdateprefix_interactive_no(self):
        ...
        self.touch(filename)
        (rc, output, error) = self.invokeAsSubprocess(
            [filename], feedInput=b"n", extraParams=["--interactive"], expectOutput=True
        )
        self.assertEqual(0, rc)
        self.assertTrue(os.path.exists(filename))
        self.assertFalse(
    ...
    def test_basicdateprefix_interactive_oneyesoneno(self):
        ...
        self.assertFalse(
            os.path.exists(
                os.path.join(self.workingDir, self.getDatePrefix() + "blah2.txt")
            )
        )
    ...
    def test_basicdateprefix_interactive_oneyesquit(self):
        ...
        self.assertFalse(
            os.path.exists(
                os.path.join(self.workingDir, self.getDatePrefix() + "blah3.txt")
            )
        )
    ...
    def test_basicdateprefix_interactive_edit(self):
        filename = os.path.join(self.workingDir, "blah.txt")
        self.touch(filename)
        with self.invokeAsPexpect(
            [filename],
            extraParams=["--interactive"],
            expectedExitStatus=0,
            expectedOutputRegex="Move " + re.escape(filename) + ".*",
        ) as child:
            child.expect("]? ")
            child.send("e")
    ...
    def test_logfile(self):
        ...
        self.touch(filename)
        self.touch(filename2)
        (rc, output, error, undo_log_lines) = self.invokeAsSubprocess(
            [filename, filename2], useUndoFile=True
        )
        self.assertEqual(0, rc)
        self.assertFalse(os.path.exists(filename))
    ...
    def test_logfile_with_spaces(self):
        ...
        self.assertTrue(
            os.path.exists(
                os.path.join(
                    self.workingDir, "sub dir", self.getDatePrefix() + "foo bar.txt"
                )
            )
        )
        self.assertEqual(
            1, self.directoryFileCount(os.path.join(self.workingDir, "sub dir"))
        )
        self.assertEqual("", error)
        self.assertEqual(0, self.executeUndoCommands(undo_log_lines))
        self.assertTrue(os.path.exists(filename))
    ...
    def test_directory_withfiles_recursive_logfile(self):
        ...
        self.touch(filename3)
        (rc, output, error, undo_log_lines) = self.invokeAsSubprocess(
            [subWorkingDir], extraParams=["--recursive"], useUndoFile=True
        )
        self.assertEqual(0, rc)
        self.assertEqual("", error)
        ...
        self.assertEqual(
            1, self.directoryFileCount(os.path.join(subWorkingDir, "subWorkingDir2"))
        )
    ...
    def test_discardprevioussuffix(self):
        ...
        self.touch(filename)
        (rc, output, error) = self.invokeAsSubprocess(
            [filename], extraParams=["--discard-existing-name"]
        )
        self.assertEqual(0, rc)
        self.assertFalse(os.path.exists(filename))
        self.assertTrue(
            os.path.exists(
                os.path.join(
                    self.workingDir, self.getDatePrefix(postfixDash=False) + ".txt"
                )
            )
        )
        self.assertEqual(1, self.directoryFileCount(self.workingDir))
