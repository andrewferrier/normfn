...
class NormalizeFilenameTestCase(unittest.TestCase):
    ...
    def getOriginalScriptPath(self):
        ...
        module_path = os.path.join(
            os.path.dirname(os.path.dirname(module_path)), "normalize-filename"
        )
    ...
    def invokeDirectly(self, inputFiles, extraParams=[]):
        ...
        loader = importlib.machinery.SourceFileLoader(
            "normalize-filename", module_path
        )
    ...
    def invokeAsSubprocess(
        ...
        with tempfile.NamedTemporaryFile() as undo_log_file:
            ...
            if feedInput:
                p = Popen(
                    options, stdin=PIPE, stdout=PIPE, stderr=PIPE, cwd=cwd
                )
            else:
                p = Popen(
                    options, stdin=None, stdout=PIPE, stderr=PIPE, cwd=cwd
                )
    ...
    def invokeAsPexpect(
        ...
        if expectedOutputRegex is not None:
            self.assertRegex(
                str(child.logfile_read.getvalue(), "utf-8"),
                expectedOutputRegex,
            )
    ...
    def tearDown(self):
        ...
        for root, dirs, files in os.walk(self.workingDir):
            ...
            for dir_and_file in dirs_and_files:
                os.chmod(
                    os.path.join(root, dir_and_file),
